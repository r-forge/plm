...
deviance.panelmodel <- function(object, model = NULL, ...){
  if (is.null(model)) sum(object$residuals^2)
...
r.squared <- function(object, model = NULL,
    ...
    if (dfcor) R2 <- 1 - (1 - R2) * (length(object$residuals) - 1) / df.residual(object)
    R2
}
...
residuals.plm <- function(object, model = NULL, effect = NULL, ...){
  ...
  if (is.null(model)) {
    ...
    object$na.action <- NULL # just to be safe: remove (newly introduced) na.action element before any other functons are called
