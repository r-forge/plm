...
# Note: commented out this lag.pseries, because there is another function with same name in this file
#
# Also note: There is also a pdiff function further down below and a bit down below a diff function
...
diff.pseries <- function(x, lag = 1, ...){
  ...
  if (lag < 0) stop("diff is only relevant for non-negative lags")
